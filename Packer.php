    ...
    public function packIntoBox(Box $aBox, ItemList $aItems) {
      ...
      while(!$aItems->isEmpty()) {
        ...
        if ($fitsSameGap >= 0 || $fitsRotatedGap >= 0) {
          ...
          $remainingWidth = $layerWidth ? min(floor($layerWidth * 1.0), $aBox->getInnerWidth()) : $aBox->getInnerWidth();
          $remainingLength = $layerLength ? min(floor($layerLength * 1.0), $aBox->getInnerLength()) : $aBox->getInnerLength();
